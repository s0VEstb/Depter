...
class ElsomParser(PDFParserBase):
    """Парсер PDF-выписок Элсом (Elsom)."""

    def can_parse(self, text: str) -> bool:
        """Проверяет наличие маркеров Элсом в тексте."""
        text_lower = text.lower()
        return any(marker in text_lower for marker in ["элсом", "elsom"])
    ...
    def parse(self, pdf_bytes: bytes) -> List[RawTransaction]:
        ...
        try:
            ...
            logger.info(f"Элсом парсер: найдено {len(transactions)} транзакций")
        except Exception as e:
            logger.error(f"Ошибка парсинга Элсом PDF: {e}")
            raise

        return transactions

    def _parse_table(self, table: list) -> List[RawTransaction]:
        """Парсинг табличных данных Элсом."""
        results = []
        ...
        for row in table[1:]:
            ...
            try:
                # Элсом: обычно [дата, описание, сумма, баланс]
                ...
                logger.debug(f"Элсом: пропуск строки: {e}")
                continue

        return results

    def _parse_text_lines(self, text: str) -> List[RawTransaction]:
        """Парсинг текстовых данных Элсом."""
        results = []
        lines = text.split("\n")
    ...
    def _detect_txn_type(description: str) -> str:
        desc_lower = description.lower()
        if any(kw in desc_lower for kw in ["qr", "элсом", "elsom"]):
            return "qr_payment"
